...
class BannerCarouselSlider extends StatelessWidget {
  ...
  CarouselSlider _bannersSuccessState(GetBannersSuccess state, BuildContext context,
    ...
    return CarouselSlider(
      options: CarouselOptions(
        ...
        viewportFraction:
            1.1, // Set the viewport fraction for visible slides
        autoPlayInterval:
            const Duration(seconds: 3), // Time between auto-plays
        autoPlayAnimationDuration:
            const Duration(seconds: 1), // Auto-play animation duration
        autoPlayCurve:
            Curves.easeInOutCubic, // Animation curve for auto-play
        scrollDirection:
            Axis.horizontal, // Horizontal scrolling for banners
      ),
      items: state.data.data!
          .map((banner) => ClipRRect(
                borderRadius: BorderRadius.circular(
                    responsive.setBorderRadius(
                        2)), // Set rounded corners for the banners
                child: CachedNetworkImage(
                  imageUrl: banner
                      .image!, // Load the banner image from the network
